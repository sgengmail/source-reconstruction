    %%% from pipline source construction in MEGtools, edited by shuang
    %%% 11-13-2018
...
%       Pratio_verb_obj =zeros(1,fwd.src.np);   % change the fwd
%       Pratio_verb_obj(fwd.src.vertno) = (verb - obj)./(obj_base + verb_base) ;
%     
%     
%       Pratio_obj_verb =zeros(1,fwd.src.np);   % change the fwd
%       Pratio_obj_verb(fwd.src.vertno) = (obj - verb)./(obj_base + verb_base) ;
...
%     
%       map_mri_verb_obj = CM_overlay_MNI((Pratio_verb_obj)',5,subjects_dir);   
%       map_mri_verb_obj(map_mri_verb_obj==0) = 0; % set the power ratio to 1 for voxels outs
%     
%     
%       map_mri_obj_verb = CM_overlay_MNI((Pratio_obj_verb)',5,subjects_dir);   
%       map_mri_obj_verb(map_mri_obj_verb==0) = 0; % set the power ratio to 1 for voxels outs
% %     
...
%     map_mri = map_mri_obj
%     
%     S.fname = fullfile(subjects_dir, exp,[subjects{n_sub} 'control_obj_theta.nii']);
%     S.dt(1) = 4;
...
% %     load(fullfile(subjects_dir , subjsubjectsorderects{n_sub} , 'norm' , 'inverse_norm.mat'))
% %     dnrm.write.prefix = 'i';
% %     spm_write_sn(S.fname,iprm,dnrm.write);
% %     
%     clear map_mri
   outputfile = 'social'; 
    ...
    S.private.dat.fname = S.fname;
    S = spm_write_vol(S,map_mri);
%     load(fullfile(subjects_dir , subjects{n_sub} , 'norm' , 'inverse_norm.mat'))
%     dnrm.write.prefix = 'i';
%     spm_write_sn(S.fname,iprm,dnrm.write);
...
%     clear map_mri
%     
%     map_mri = map_mri_obj_verb
%     
%     S.fname = fullfile(subjects_dir, exp,[subjects{n_sub} 'control_obj_verb.nii']);
%     S.dt(1) = 4;
%     S.pinfo(1) = max(map_mri(:))/(2^15-1);
%     S.pinfo(2) = 0;
%     S.pinfo(3) = 0;
